...
int main(int argc, char **argv)
{
	...
	if (my_rank == 0)
	{
		...
		data_file = fopen("./data/vector_gather_time.txt", "a");
...
int sum_vector_p(int vector[], int size, int my_rank, int uni_size)
{
	...
	// Initialise a vector to take the sums from each process
	int* local_sums = malloc(uni_size * sizeof(int));
	...
	MPI_Gather(&partial_sum, count, MPI_INT, local_sums, count, MPI_INT, root, MPI_COMM_WORLD);
