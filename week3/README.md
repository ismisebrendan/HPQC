...
## Files in this directory
 ...
 - ```vector_serial_internal_overhead.c``` - The ```vector_serial_internal.c``` file edited so that it outputs the time taken to run the entire program, as found internally.
 ...
 - ```vector_parallel_internal_overhead.c``` - Sum the vector in parallel and time the entire process internally.  [AN MPI FILE]
