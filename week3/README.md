...
## Files in this directory
 ...
 - ```hello_world.c``` - Hello world in serial. Takes an integer as an input.
 - ```timing_results.txt``` - The results from timing the different files. (Results also shown below).
 - ```proof.c``` - The ```proof.c``` file copied from the course repository. Takes an integer as an input. [AN MPI FILE]
 - ```vector_serial.c``` - The ```vector_serial.c``` file copied from the course repository. Takes an integer as an input.
 - ```vector_serial_edited.c``` - The ```vector_serial.c``` file edited so that it sets the ith component of the vector to $i^2$. Takes an integer as an input.
 - ```vector_serial_internal.c``` - The ```vector_serial_edited.c``` file edited so that it outputs the time taken to run the program, as found internally. Takes an integer as an input.
 - ```vector_serial_internal_overhead.c``` - The ```vector_serial_internal.c``` file edited so that it outputs the time taken to run the entire program, as found internally. Takes an integer as an input.
 ...
 - ```vector_parallel.c``` - Sum the vector in parallel. Takes an integer as an input. [AN MPI FILE]
 - ```vector_parallel_internal.c``` - Sum the vector in parallel and time the process internally. Takes an integer as an input. [AN MPI FILE]
 - ```vector_parallel_internal_overhead.c``` - Sum the vector in parallel and time the entire process internally. Takes an integer as an input. [AN MPI FILE]
 ...
 - ```time_internal_serial_parallel_overhead.sh``` - Runs ```vector_serial_internal_overhead.c``` and ```vector_parallel_internal_overhead.c``` repeatedly for a number of different inputs and number of processes in order to find the time taken to run for a number of different inputs.
